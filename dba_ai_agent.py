...
Format the response in a clear, structured way."""
    ...
    def apply_fix_suggestions(self, conn_name: str, insights: Dict) -> Dict:
        """
        Apply suggested fixes to database tables based on insights
        ...
        Returns:
            Dictionary with results of applied fixes
        """
        results = {
            "applied_fixes": [],
            "warnings": []
        ...
        try:
            ...
            # Handle tables with high sequential scans
            if 'index_recommendations' in insights_data and isinstance(insights_data['index_recommendations'], list):
                for rec in insights_data['index_recommendations']:
                    table_name = rec.get('table_name')
                    seq_scan = rec.get('seq_scan', 0)
                    ...
                    # Only consider tables with significant sequential scan to index scan ratio
                    if table_name and seq_scan > 100 and (idx_scan == 0 or (seq_scan / max(idx_scan, 1)) > 10):
                        # Get table columns to identify potential index candidates
                        columns_query = f"""
                        ...
                        """
                        success, columns_result, _ = conn.execute_query(columns_query)
                        
                        if success and columns_result:
                            # Analyze query patterns to find the most frequently used columns in WHERE clauses
                            query_patterns = self.analyze_query_patterns(conn_name, table_name)
                            
                            if query_patterns.get('recommended_indexes'):
                                for index_rec in query_patterns.get('recommended_indexes', []):
                                    column = index_rec.get('column')
                                    ...
                                    if column:
                                        ...
                                        """
                                        success, index_check_result, _ = conn.execute_query(index_check_query)
                                        
                                        if success and not index_check_result:
                                            # Create the index
                                            ...
                                            success, _, _ = conn.execute_query(create_index_query)
                                            ...
                                            if success:
                                                ...
                                                results["warnings"].append(f"Failed to create index on {table_name}({column})")
                                        elif success and index_check_result:
                                            results["warnings"].append(f"Index already exists on {table_name}({column})")
                            else:
                                # Fallback to creating indexes on primary key or id columns if no query patterns found
                                for col in columns_result:
                                    col_name = col[0]
                                    data_type = col[1]
                                    
                                    # Look for potential key columns
                                    if col_name.lower() in ['id', 'user_id', 'player_id', 'activity_id', 'event'] and data_type in ['integer', 'bigint', 'uuid']:
                                        # Check if index already exists
                                        index_check_query = f"""
                                            SELECT indexname 
                                            FROM pg_indexes 
                                            WHERE tablename = '{table_name}' 
                                            AND indexdef LIKE '%({col_name})%';
                                        """
                                        success, index_check_result, _ = conn.execute_query(index_check_query)
                                        ...
                                        if success and not index_check_result:
                                            ...
                                            if success:
                                                ...
                                                logger.info(f"Created index {index_name} on {table_name}({col_name})")
                                            else:
                                                results["warnings"].append(f"Failed to create index on {table_name}({col_name})")
                                        elif success and index_check_result:
                                            results["warnings"].append(f"Index already exists on {table_name}({col_name})")
            
            # Handle very large tables
            if 'largest_tables' in insights_data and isinstance(insights_data['largest_tables'], list):
                for table in insights_data['largest_tables']:
                    table_name = table.get('table_name')
                    total_size_bytes = table.get('total_size_bytes', 0)
                    
                    # For extremely large tables (>100GB), recommend partitioning
                    if table_name and total_size_bytes > 100 * 1024 * 1024 * 1024:  # >100GB
                        results["warnings"].append(f"Table {table_name} is extremely large ({table.get('total_size_pretty')}). Consider implementing partitioning.")
                        
                        # Check if table has a timestamp or date column that could be used for partitioning
                        date_column_query = f"""
                            SELECT column_name 
                            FROM information_schema.columns 
                            WHERE table_name = '{table_name}' 
                        ...
                        """
                        success, date_columns, _ = conn.execute_query(date_column_query)
                        
                        if success and date_columns:
                            potential_partition_columns = [col[0] for col in date_columns]
                            results["warnings"].append(f"Potential partition columns for {table_name}: {', '.join(potential_partition_columns)}")
                    
                    # For large tables (>10GB), perform VACUUM ANALYZE
                    elif table_name and total_size_bytes > 10 * 1024 * 1024 * 1024:  # >10GB
                        # Check when the table was last analyzed
                        last_analyze_query = f"""
                            SELECT last_analyze 
                            FROM pg_stat_user_tables 
                            WHERE relname = '{table_name}';
                        """
                        success, last_analyze_result, _ = conn.execute_query(last_analyze_query)
                        
                        if success and last_analyze_result:
                            last_analyze = last_analyze_result[0][0]
                            
                            # If the table hasn't been analyzed in the last 7 days or never analyzed
                            if not last_analyze or (datetime.now() - last_analyze).days > 7:
                                # Get the size before vacuum
                                size_query = f"""
                                    SELECT pg_size_pretty(pg_total_relation_size('{table_name}'));
                                """
                                success, size_result, _ = conn.execute_query(size_query)
                                size_before = size_result[0][0] if success and size_result else "Unknown"
                                
                                # Perform VACUUM ANALYZE
                                vacuum_query = f"VACUUM ANALYZE {table_name};"
                                success, _, _ = conn.execute_query(vacuum_query)
                                ...
                                if success:
                                    ...
                                    logger.info(f"Performed VACUUM ANALYZE on {table_name}")
                                else:
                                    results["warnings"].append(f"Failed to perform VACUUM ANALYZE on {table_name}")
            ...
            if 'slow_queries' in insights_data and isinstance(insights_data['slow_queries'], list):
                for query in insights_data['slow_queries']:
                    ...
                    # For extremely slow queries (>1000ms), log a warning
                    if query_text and mean_time > 1000:
                        truncated_query = query_text[:100] + '...' if len(query_text) > 100 else query_text
                        results["warnings"].append(f"Extremely slow query detected: {truncated_query} (Avg time: {mean_time}ms)")
                        
                        # If it's a replication query, add a specific warning
                        if 'REPLICATION' in query_text.upper():
                            results["warnings"].append("Replication query performance issues detected. Consider reviewing replication configuration.")
            
            return results
            
        except Exception as e:
            logger.error(f"Error applying fix suggestions: {str(e)}")
